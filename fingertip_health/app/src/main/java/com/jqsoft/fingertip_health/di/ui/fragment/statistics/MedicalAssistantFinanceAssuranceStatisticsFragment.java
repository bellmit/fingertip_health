...
import com.jqsoft.fingertip_health.adapter.statistics.MedicalAssistantFinanceAssuranceStatisticsAdapter;
...
import com.jqsoft.fingertip_health.di.presenter.MedicalAssistantFinanceAssuranceStatisticsFragmentPresenter;
...
public class MedicalAssistantFinanceAssuranceStatisticsFragment extends AbstractFragment implements MedicalAssistantFinanceAssuranceStatisticsFragmentContract.View, IUpLevel {
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<MedicalAssistantFinanceAssuranceBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList, totalList);
        //显示列表
...
//        hbcChart.getAxisRight().setAxisMaximum(moneyMaxReal);
        ...
        hbcChart.setVisibleYRangeMinimum(Util.getMax(personNumberList)*Constants.CHART_Y_AXIS_VALUE_FACTOR,
                YAxis.AxisDependency.RIGHT);
        hbcChart.setVisibleYRangeMinimum(Util.getMax(moneyList)*Constants.CHART_Y_AXIS_VALUE_FACTOR,
    ...
    private MedicalAssistantFinanceAssuranceBean getTotalBeanFromTotalList(List<String> totalList){
        if (ListUtils.isEmpty(totalList) || ListUtils.getSize(totalList)<2 ||
                !Constants.MEDICAL_ASSISTANT_FINANCE_ASSURANCE_RANKING_VALUE.equals(type)){
            return null;
        } else {
        ...
        try {
            if (ListUtils.isEmpty(rawList) ||
                ...
                if (is5Column()) {
                    ...
                    MedicalAssistantFinanceAssuranceBean total = getTotalBeanFromTotalList(totalList);
                    if (total!=null){
