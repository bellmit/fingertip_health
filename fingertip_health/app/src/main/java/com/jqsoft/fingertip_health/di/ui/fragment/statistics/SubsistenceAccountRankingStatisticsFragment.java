...
import com.jqsoft.fingertip_health.adapter.statistics.SubsistenceAccountRankingStatisticsAdapter;
...
import com.jqsoft.fingertip_health.di.presenter.SubsistenceAccountRankingStatisticsFragmentPresenter;
...
public class SubsistenceAccountRankingStatisticsFragment extends AbstractFragment implements SubsistenceAccountRankingStatisticsFragmentContract.View, IUpLevel {
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<SubsistenceAccountRankingNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList, totalList);
        //显示列表
//        adapter.setTitleList(titleList);
        adapter.setNewData(naturalList);
        showSuccessViewOrFailureView(true, ListUtils.isEmpty(adapter.getData()));
...
//        hbcChart.setVisibleXRangeMaximum(Constants.BAR_NUMBER_PER_SCREEN_SMALL*barWidth);
    ...
    private SubsistenceAccountRankingNaturalBean getTotalBeanFromTotalList(List<String> totalList){
        if (ListUtils.isEmpty(totalList) || ListUtils.getSize(totalList)<1){
            ...
            nb.setMoney(Util.getNonscientificNumberStringFromString(totalList.get(0)));
            return nb;
        }
...
//                for (int i = 0; i < ListUtils.getSize(rawList); ++i){
                SubsistenceAccountRankingNaturalBean total = getTotalBeanFromTotalList(totalList);
                if (total!=null){
