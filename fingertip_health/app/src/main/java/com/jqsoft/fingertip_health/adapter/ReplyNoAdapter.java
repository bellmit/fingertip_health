...
//咨询建议适配器
public class ReplyNoAdapter extends BaseQuickAdapterEx<AdviceBean, BaseViewHolder> {
    ...
    public ReplyNoAdapter(List<AdviceBean> data, int type) {
...
//        this.context=context;
    ...
    protected void convert(final BaseViewHolder helper, final AdviceBean item) {
        ...
        helper.setText(R.id.tv_content, "咨询内容: "+Util.trimString(item.getContent()));
        if (!TextUtils.isEmpty(item.getReplyTime())){
            helper.setText(R.id.tv_time,"咨询时间: " + Util.trimString(item.getReplyTime().replace('T',' ')));
        }else {
