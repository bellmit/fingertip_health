...
import com.jqsoft.fingertip_health.adapter.statistics.SubsistenceArchiveRankingStatisticsAdapter;
...
import com.jqsoft.fingertip_health.di.presenter.SubsistenceArchiveRankingStatisticsFragmentPresenter;
...
public class SubsistenceArchiveRankingStatisticsFragment extends AbstractFragment implements SubsistenceArchiveRankingStatisticsFragmentContract.View, IUpLevel {
...
//        des.setText(Constants.CHART_LARGE_VALUE_HINT_TEXT);
//        des.setTextColor(Color.RED);
//        des.setTextAlign(Paint.Align.CENTER);
//        hbcChart.setDescription(des);
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<SubsistenceArchiveRankingNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList, totalList);
        //显示列表
//        adapter.setTitleList(titleList);
        adapter.setNewData(naturalList);
        showSuccessViewOrFailureView(true, ListUtils.isEmpty(adapter.getData()));
...
//        hbcChart.setVisibleXRangeMaximum(Constants.NUMBER_PER_DISTRICT * yVals1.size() * barWidth);
    ...
    private SubsistenceArchiveRankingNaturalBean getTotalBeanFromTotalList(List<String> totalList){
        if (ListUtils.isEmpty(totalList) || ListUtils.getSize(totalList)<2){
            ...
            nb.setHouseholdNumber(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(totalList.get(0))));
            nb.setPersonNumber(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(totalList.get(1))));
            return nb;
        }
    }
...
//                for (int i = 0; i < ListUtils.getSize(rawList); ++i){
                SubsistenceArchiveRankingNaturalBean total = getTotalBeanFromTotalList(totalList);
                if (total!=null){
...
//                    SubsistenceVarianceRankingSpecificCategoryBean bean2 = rawList.get(2);
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        ...
                        nb.setHouseholdNumber(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(bean0.getList().get(j))));
                        nb.setPersonNumber(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(bean1.getList().get(j))));
