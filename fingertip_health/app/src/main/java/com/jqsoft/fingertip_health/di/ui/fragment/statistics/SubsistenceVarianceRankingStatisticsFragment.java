...
import com.jqsoft.fingertip_health.adapter.statistics.SubsistenceVarianceRankingStatisticsAdapter;
...
import com.jqsoft.fingertip_health.di.presenter.SubsistenceVarianceRankingStatisticsFragmentPresenter;
...
public class SubsistenceVarianceRankingStatisticsFragment extends AbstractFragment implements SubsistenceVarianceRankingStatisticsFragmentContract.View, IUpLevel {
...
//        });
        ...
        RxView.clicks(tvSubsistenceType)
                ...
                .subscribe(new Observer<Object>() {
                    ...
                    @Override
                    public void onNext(Object o) {
                        String[] typeArray = new String[]{"农村低保", "城市低保"};
//                        String[] typeArray = new String[]{"全部", "农村低保", "城市低保"};
                        List<String> typeList = Arrays.asList(typeArray);
                        Util.showSingleChoiceStringListMaterialDialog(getActivity(), "请选择户籍类别", null, typeList,
...
//        float randomMultiplier = 3* 100000f;
        final List<String> titleList = bean.getxTitle();
//        currentAreaCodeList=Arrays.asList("340103007");
        ...
        List<SubsistenceVarianceRankingNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList, totalList);
        //显示列表
//        adapter.setTitleList(titleList);
        adapter.setNewData(naturalList);
        showSuccessViewOrFailureView(true, ListUtils.isEmpty(adapter.getData()));
...
//                f=0;
                if (j == 0) {
                    ...
                    yVals2.add(new BarEntry(i, f));

                } else if (j == 2) {
                    yVals3.add(new BarEntry(i, f));
...
////        float axisMax = hbcChart.getAxisLeft().getAxisMaximum();
////        float ultimateMax = axisMax;
////        if (axisMax < 30){
////            ultimateMax = 30;
////        }
////        hbcChart.getAxisLeft().setAxisMaximum(ultimateMax);
////        hbcChart.getAxisRight().setAxisMaximum(ultimateMax);



//        hbcChart.setVisibleXRangeMaximum(Constants.NUMBER_PER_DISTRICT * yVals1.size() * barWidth);
//        hbcChart.setVisibleXRangeMaximum(Constants.BAR_NUMBER_PER_SCREEN_FOR_ONE_COLUMN*barWidth);
//        hbcChart.setVisibleXRangeMaximum(36*barWidth);
...
//        hbcChart.setVisibleXRangeMinimum(Constants.BAR_NUMBER_PER_SCREEN*barWidth);
    ...
    private SubsistenceVarianceRankingNaturalBean getTotalBeanFromTotalList(List<String> totalList){
        if (ListUtils.isEmpty(totalList) || ListUtils.getSize(totalList)<3){
            return null;
        } else {
            SubsistenceVarianceRankingNaturalBean nb = new SubsistenceVarianceRankingNaturalBean();
            nb.setDistrictName(Constants.LIST_SUMMARY_TEXT);
            nb.setNewQuantity(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(totalList.get(0))));
            nb.setReviewQuantity(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(totalList.get(1))));
            nb.setDeleteQuantity(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(totalList.get(2))));
            return nb;
        }
    }
...
//                for (int i = 0; i < ListUtils.getSize(rawList); ++i){
                SubsistenceVarianceRankingNaturalBean total = getTotalBeanFromTotalList(totalList);
                if (total!=null){
                    ...
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        ...
                        nb.setNewQuantity(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(bean0.getList().get(j))));
                        nb.setReviewQuantity(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(bean1.getList().get(j))));
                        nb.setDeleteQuantity(Util.getCommaSeparatedIntString(Util.getForcedIntStringFromDoubleString(bean2.getList().get(j))));
                        resultList.add(nb);
                    }
//                }
            }
